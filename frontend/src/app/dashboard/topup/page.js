      ...
      <div className="page-content">
        <div style={styles.container}>
          <div style={styles.header}>
            <h2 style={styles.title}>💳 Top-Up Saldo</h2>
            <p style={styles.subtitle}>
              Saldo saat ini: <strong style={{ color: '#a5b4fc' }}>
          ...
          <form onSubmit={handleSubmit} style={styles.form}>
            <div style={styles.fieldGroup}>
              ...
              {amount && estimateUploads > 0 && (
                <p style={styles.estimate}>
                  ≈ {estimateUploads} upload (@ Rp {uploadPrice.toLocaleString('id-ID')}/upload)
                </p>
              )}
            </div>

            <div style={styles.fieldGroup}>
              <label style={styles.label}>📎 Bukti Transfer (JPG/PNG/PDF, maks 10MB)</label>
              <input id="proof-file-input" type="file" accept="image/*,.pdf"
                onChange={e => setProofFile(e.target.files?.[0] || null)}
                style={styles.fileInput} />
              {proofFile && (
                <p style={{ ...styles.estimate, marginTop: '0.3rem' }}>
                  ✅ {proofFile.name}
                </p>
              )}
            </div>

            <button type="submit" disabled={submitting || !amount || !proofFile}
              style={{ ...styles.submitBtn, opacity: (submitting || !amount || !proofFile) ? 0.6 : 1 }}>
              {submitting ? '⏳ Mengirim...' : '🚀 Kirim Request Top-Up'}
            </button>
          </form>

          {/* CS Contact */}
          {(settings.cs_whatsapp || settings.cs_email) && (
            <div style={styles.csCard}>
              <h3 style={styles.csTitle}>📞 Butuh bantuan?</h3>
              {settings.cs_whatsapp && (
...
const styles = {
  ...
  title: { color: '#fff', margin: '0 0 0.25rem', fontSize: '1.6rem' },
